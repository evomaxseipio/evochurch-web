-- =============================================================================
-- EvoChurch — spgetprofiles paginado (SOBRECARGA, no reemplaza la versión vieja)
-- Ejecutar TODO en Supabase SQL Editor → Run (sin límite de 100 filas)
-- =============================================================================
...
BEGIN
  ...
  WITH scoped AS (
    ...
    WHERE (p_church_id IS NULL OR p.church_id = p_church_id)
  ),
  stats AS (
    SELECT
  ...
  SELECT json_build_object(
    ...
    'pagination', json_build_object(
      'page', v_page,
      'page_size', COALESCE(v_page_size, v_total),
      'total', v_total,
      'total_pages', CASE
        WHEN v_page_size IS NULL OR v_page_size = 0 THEN 1
        ELSE GREATEST(CEIL(v_total::numeric / v_page_size::numeric), 1)
      END
    ),
    'stats', (
      SELECT json_build_object(
...
GRANT EXECUTE ON FUNCTION public.spgetprofiles(
