...
function MembersScreen({ onOpen, toast }) {
  ...
  const [pageSize, setPageSize] = useStateB(8);
  ...
  return (
    <div onClick={() => setActionMenuFor(null)}>
      <div className="row between" style={{ flexWrap: "wrap", gap: 16 }}>
        ...
        <div className="row">
          <button className="btn outline" onClick={() => toast?.("success", "Reporte exportado", "Miembros_May2026.xlsx descargado")}>
            <Ic.download width={16}/> Exportar
          </button>
          <button className="btn primary" onClick={() => setDrawerOpen(true)}>
            <Ic.plus width={16}/> Nuevo miembro
          </button>
        </div>
      </div>
      ...
      <div className="grid-12" style={{ marginTop: 22 }}>
        {[
          ...
          { l: "Activos", v: stats.activos, c: "var(--success)" },
          { l: "Inactivos", v: stats.inactivos, c: "var(--ink-3)" },
          { l: "Pendientes", v: stats.pendientes, c: "var(--accent-600)" },
        ].map((s, i) => (
          <div key={i} className="span-3 card" style={{ padding: 18 }}>
            <div className="eyebrow">{s.l}</div>
            <div className="row between" style={{ alignItems: "flex-end", marginTop: 8 }}>
              <div className="display" style={{ fontSize: 36, lineHeight: 1, color: s.c }}>{s.v}</div>
              <div style={{ width: 4, height: 32, background: s.c, borderRadius: 4, opacity: 0.6 }}/>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Filter row */}
      <div className="card flat" style={{ marginTop: 18, padding: 14, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button className="btn outline sm"><Ic.filter width={14}/> Más filtros</button>
      </div>
                ...
                <td>
                  <div className="row" style={{ gap: 8 }}>
                    <div style={{ width: 80, height: 6, background: "var(--surface-2)", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ width: m.attendance + "%", height: "100%",
                        background: m.attendance > 80 ? "var(--success)" : m.attendance > 50 ? "var(--warning)" : "var(--danger)" }}/>
                    </div>
                    <span className="tnum tiny" style={{ fontWeight: 600 }}>{m.attendance}%</span>
                  </div>
                </td>
                <td className="tnum mono" style={{ fontSize: 13 }}>{fmtRD(m.given)}</td>
                <td>
  ...
  return (
    <div style={{ position: "relative", display: "inline-block" }} ref={ref}>
      <button
        ...
        style={{ padding: "4px 9px", gap: 5 }}
      >
        Actions <Ic.arrowDn width={12}/>
      </button>
      {open && (
        <div
...
// Pagination
// ============================================================
function Pagination({ page, totalPages, total, pageStart, pageEnd, pageSize, onPage, onPageSize }) {
    ...
    >
      <div className="row" style={{ gap: 12, alignItems: "center" }}>
        <span className="tiny muted">
          ...
          {" de "}
          <b style={{ color: "var(--fg)", fontFamily: "var(--font-mono)" }}>{total.toLocaleString("es-DO")}</b>{" "}miembros
        </span>
        ...
        <label className="row" style={{ gap: 6, fontSize: 12, color: "var(--muted)" }}>
          ...
          >
            {[8, 12, 24, 50].map(n => <option key={n} value={n}>{n}</option>)}
          </select>
        </label>
      </div>
...
function MemberDrawer({ onClose, toast }) {
  ...
  const [sector, setSector] = useStateB("");
  return (
    <>
      ...
      <div className="drawer">
        ...
        <div className="drawer-body col gap-md">
          <div className="row" style={{ gap: 14, alignItems: "center", padding: 16, background: "var(--primary-50)", borderRadius: 14 }}>
            <Avatar initials={name ? name.split(" ").map(s=>s[0]).slice(0,2).join("").toUpperCase() : "??"} size="lg" square/>
            <div>
              <div style={{ fontWeight: 600 }}>{name || "Nombre del nuevo miembro"}</div>
          ...
          <div className="field">
            ...
            <div className="input-wrap"><input value={name} onChange={e=>setName(e.target.value)} placeholder="Ej. María Altagracia Peña"/></div>
          </div>
          <div className="row" style={{ gap: 12 }}>
            <div className="field" style={{ flex: 1 }}>
              <label>Teléfono</label>
              <div className="input-wrap"><input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="(809) 555-0000"/></div>
            </div>
            <div className="field" style={{ flex: 1 }}>
              <label>Sector</label>
              <div className="input-wrap"><input value={sector} onChange={e=>setSector(e.target.value)} placeholder="Los Jardines"/></div>
            </div>
          </div>
          <div className="field">
            <label>Rol en la iglesia</label>
            <div className="input-wrap">
              <select defaultValue="Miembro"><option>Miembro</option><option>Diácono</option><option>Diaconisa</option><option>Líder de Jóvenes</option><option>Coro</option><option>Tesorero</option></select>
            </div>
          </div>
          <div className="field">
            <label>Estado</label>
            <div className="row" style={{ gap: 8, padding: 4, background: "var(--surface-2)", borderRadius: 10, width: "fit-content" }}>
              {["Activo","Pendiente","Inactivo"].map((s, i) => (
          ...
          <div className="field">
            ...
            <div className="input-wrap" style={{ alignItems: "flex-start", padding: "10px 12px" }}>
              <textarea rows={3} placeholder="Anotaciones, cumpleaños, oración…"/>
            </div>
          </div>
        </div>
        <div className="drawer-foot">
          <button className="btn outline" onClick={onClose}>Cancelar</button>
          <button className="btn primary" onClick={() => { onClose(); toast?.("success", "Miembro agregado", (name || "Nuevo miembro") + " ya forma parte de Renacer."); }}>
