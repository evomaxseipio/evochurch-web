...
export async function createMemberAction(
  ...
  try {
    ...
    const result = await insertMember(supabase, churchId, input);
    const newProfileId = String(
      result.profile_id ?? result.profileId ?? result.id ?? "",
    ).trim();

    const parentProfileId = String(formData.get("parentProfileId") ?? "").trim();
    if (parentProfileId && newProfileId) {
      const relRaw = String(formData.get("familyRelationship") ?? "").trim();
      const relationship = isFamilyChildRelationship(relRaw)
        ? relRaw
        : familyRelationshipForGender(input.gender || "");
