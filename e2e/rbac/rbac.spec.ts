...
test.describe("RBAC-AUTH / ROUTE", () => {
  test("RBAC-AUTH-01 U-ADMIN lands on dashboard", async ({ page }) => {
    ...
    await page.goto(churchPath("/dashboard"));
    await expect(page).toHaveURL(/\/apps\/church\/dashboard/);
  });
  ...
  test("RBAC-AUTH-01 U-NOROLE limited home", async ({ page }) => {
    ...
    await page.goto(churchPath("/settings"));
    await expect(page).toHaveURL(/\/apps\/church\/settings/);
  });
...
test.describe("RBAC NAV sidebar", () => {
  test("NAV-01 U-ADMIN full nav", async ({ page }) => {
    ...
    expect(await sidebarHrefVisible(page, churchPath("/settings/users"))).toBeTruthy();
  });
  ...
  test("NAV-02 U-TESORERO finances without admin users", async ({ page }) => {
    ...
    expect(await sidebarHrefVisible(page, churchPath("/finances/transactions"))).toBeTruthy();
    expect(await sidebarHrefVisible(page, churchPath("/settings/users"))).toBeFalsy();
  });
  ...
  test("NAV-03 U-LIDER ministries without finances", async ({ page }) => {
    ...
    await page.goto(churchPath("/ministerios"));
    expect(await sidebarHrefVisible(page, churchPath("/ministerios"))).toBeTruthy();
    expect(await sidebarHrefVisible(page, churchPath("/finances/transactions"))).toBeFalsy();
    expect(await sidebarHrefVisible(page, churchPath("/finances/contributions"))).toBeFalsy();
  });
  ...
  test("NAV-04 U-NOROLE minimal nav", async ({ page }) => {
    ...
    await loginAs(page, "NOROLE");
    await page.goto(churchPath("/settings"));
    expect(await sidebarHrefVisible(page, churchPath("/members"))).toBeFalsy();
    expect(await sidebarHrefVisible(page, churchPath("/finances/transactions"))).toBeFalsy();
  });
});
...
test.describe("RBAC UI actions", () => {
  test("UI-01 U-TESORERO sees transactions page", async ({ page }) => {
    test.skip(!credsFor("TESORERO"), "U-TESORERO creds missing");
    await loginAs(page, "TESORERO");
    await page.goto(churchPath("/finances/transactions"));
...
test.describe("RBAC REG regression smoke", () => {
  ...
  test("REG-02 U-ADMIN contributions page 2", async ({ page }) => {
    test.skip(!credsFor("ADMIN"), "U-ADMIN creds missing");
    await loginAs(page, "ADMIN");
    await page.goto(`${churchPath("/finances/contributions")}?page=2`);
